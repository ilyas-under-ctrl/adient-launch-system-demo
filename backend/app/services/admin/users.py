...
def create_user(db: Session, payload: UserCreate) -> User:
    ...
    user = User(**payload.model_dump(exclude={"password"}), password_hash=hash_password(payload.password), status=RecordStatus.ACTIVE)
...
def set_active(db: Session, user: User, active: bool) -> User:
    if not active: ensure_not_bootstrap(user)
